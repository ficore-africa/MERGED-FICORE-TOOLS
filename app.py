...
class QuizForm(FlaskForm):
    ...
    def __init__(self, questions=None, language='en', formdata=None, **kwargs):
        ...
        # Create and bind fields without processing data
        ...
        if 'quiz_data' in session:
            for q in self.questions:
                field_name = q['id']
                if field_name in session['quiz_data']:
                    self._fields[field_name].data = session['quiz_data'][field_name]
                    logger.debug(f"Pre-populated {field_name} with {session['quiz_data'][field_name]}")
...
def quiz_step3():
    ...
    preprocessed_questions = [
        {
            'id': f'question_{i+8}',  # Questions 8-10
            'text': trans.get(q['text'], q['text']),
            'type': q['type'],
            'options': [trans.get(opt, opt) for opt in q['options']],
            'required': q.get('required', True)
